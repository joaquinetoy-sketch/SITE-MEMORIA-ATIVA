      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-6">
          {steps.map((step, index) => (
            <div key={index} className="flex flex-col items-center text-center p-6 rounded-xl border border-brand-border hover:border-brand-primary/50 transition-colors bg-white group h-full">
              <div className="w-14 h-14 rounded-full bg-brand-surface flex items-center justify-center mb-6 text-brand-dark group-hover:bg-brand-dark group-hover:text-white transition-colors shrink-0">
                <step.icon size={24} />
              </div>
              <h3 className="font-bold text-base text-brand-dark mb-3 leading-tight">{step.title}</h3>
